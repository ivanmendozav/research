...
library(readxl)
bicis <- read_excel("C:/Users/joe-a/OneDrive/Documents/U del Azuay/TRABAJO/Tesis final/PROYECTO UDA ESTUDIANTES BICICLETA/3. Análisis finales R con Man Whitman/DatosBicis.xlsx",
                    sheet = 'Bp 2')
View(bicis)
...
abline(v=mean(bicis$Edad), col="red", lwd= 3) #visualización media en el histograma
abline(v=median(bicis$Edad), col="black" , lwd= 3) #visualización mediana en el histograma
#se debe usar moda  no es una distribución normal 
legend("topright",col=c("black","red"),legend =c("Mediana","Media"),lwd=3, bty = "n")


#Velocidad
histograma<-hist(bicis$Vel_prom, main="BP Velocidad Promedio", col="#FFE1FF", ylim=c(0,20), xlab="Velocidad (km/h)",ylab="Frecuencia", breaks =15 , xaxt = "n")
axis(1, at = c(5,10,15,20,25,30,35))
abline(v=mean(bicis$Vel_prom), col="red", lwd= 3) #visualización media en el histograma
abline(v=median(bicis$Vel_prom), col="black", lwd= 3) #visualización mediana en el histograma
legend("topright",col=c("black","red"),legend =c("Mediana","Media"),lwd=3, bty = "n")
#distribución normal sesgo a la izq, se puede usar moda, no media por que hay un outlier que puede afectar

#Aceleración
histograma<-hist(bicis$aceleración_promedio, main="BP Aceleración Promedio", col="#DDA0DD", ylim=c(0,40), xlab="Aceleración (m/s²)",ylab="Frecuencia",  breaks = 10, xaxt = "n")
axis(1, at = c(0.2,0.4,0.6,0.8,1,1.2,1.4))
abline(v=mean(bicis$aceleración_promedio), col="red", lwd= 3) #visualización media en el histograma
abline(v=median(bicis$aceleración_promedio), col="black", lwd= 3) #visualización mediana en el histograma
legend("topright",col=c("black","red"),legend =c("Mediana","Media"),lwd=3, bty = "n")
#distribución normal sesgo a la izq, se puede usar moda, no media por que hay un outlier que puede afectar

#Tiempo
histograma<-hist(bicis$Tiempo, main="BP Tiempo de recorrido", col="#A4D3EE", ylim=c(0,30), xlab="Tiempo (min)",ylab="Frecuencia", breaks = 15, xaxt = "n")
axis(1, at = c(5,10,15,20,25,30,35,40,45,50))
abline(v=mean(bicis$Tiempo), col="red", lwd= 3) #visualización media en el histograma
abline(v=median(bicis$Tiempo), col="black", lwd= 3) #visualización mediana en el histograma
legend("topright",col=c("black","red"),legend =c("Mediana","Media"),lwd=3, bty = "n")
...
#Distancia  
media_dis<-mean(bicis$Distancia)
mediana_dis<-median(bicis$Distancia)
Minimo_dis<-min(bicis$Distancia)
maximo_dis<-max(bicis$Distancia)
summary(bicis$Distancia)
#para detectar la moda
Mode(bicis$Distancia) #Directamente nos entrega la moda
...
#se debe usar moda  no es una distribución normal 
legend("topright",col=c("blue","red"),legend =c("Densidad normal estimada","Estimador de núcleo de la densidad"),lwd=2, bty = "n")
...
#Aceleración
histograma<-hist(bicis$aceleración_promedio, main="Aceleración Promedio", col="violet", ylim=c(0,35), xlab="Aceleración (m/s²)", breaks = 20)
abline(v=mean(bicis$aceleración_promedio), col="red") #visualización media en el histograma
abline(v=median(bicis$aceleración_promedio), col="blue") #visualización mediana en el histograma
#distribución normal sesgo a la izq, se puede usar moda, no media por que hay un outlier que puede afectar
...
qplot(factor(Tipo_bici),acel , data = bicis, geom = "violin", stackdir = "center", binaxis = "y",
      color = factor(Tipo_bici), fill = factor(Tipo_bici),main = "Aceleración en función del tipo de bicicleta",
      xlab = "Tipo bicicleta", ylab = "Aceleración (m/s²)")
